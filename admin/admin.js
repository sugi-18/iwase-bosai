...
async function loadParticipants() {
    ...
    data.forEach(
        participant => {
             ...
             *
             * この仕組みを入れる前に登録した方は
             * member_type が入っていないので、
             * 自治会員として扱う。
             */

            const memberType =
                participant.member_type === "guest"
                    ? "guest"
                    : "member";


            tr.dataset.memberType = memberType;
            ...
            tr.innerHTML = `
                ...
                <td>
                    <span
                        class="member-type-badge ${memberType}"
                    >
                        ${
                            memberType === "guest"
                                ? "会員以外"
...
(function () {
    ...
    window.openTrainingQrModal =
        function (training) {
             ...
             * QRが読めなかった方は、この4桁を
             * アプリに入力して参加登録できる。
             * 発行と表示は community-admin.js が行う。
             */

            if (
                typeof window.IwaseFillAccessCode ===
                "function"
            ) {

                window.IwaseFillAccessCode(
                    trainingId
                );
     ...
     * QRコードの行き先の基準
     *
     * 空のままなら、管理者画面と同じ場所を基準にする。
     *
     * 利用者が使うアプリと管理者画面が
     * 別のアドレスで公開されている場合は、
     * ここにアプリ側のアドレスを入れる。
     *
     *   例）"https://sugi-18.github.io/iwase-bosai/"
     *
     * 行き先がアプリと違うと、利用者情報が
     * 引き継がれず参加登録に失敗することがある。
     * QRを作ったら、一度スマートフォンで読み取り、
     * アドレスがアプリと同じか確認すること。
     */

    const APP_BASE_URL =
        "https://sugi-18.github.io/iwase-bosai/";
