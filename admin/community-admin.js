...
async function loadPlanImages() {
    ...
    const container = el("planImageSlots");
    ...
    try {
        ...
        for (let i = 1; i <= PLAN_MAX_IMAGES; i++) {

            container.appendChild(
                createPlanSlot(i, row["image" + i + "_url"])
            );
...
function createPlanSlot(number, url) {
    ...
    input.addEventListener(
        ...
        async event => {
            ...
            if (file) {
                ...
                await uploadPlanImage(number, file);
...
async function uploadPlanImage(number, file) {
    ...
    try {
        ...
        const column = "image" + number + "_url";
...
async function deletePlanImage(number, url) {
